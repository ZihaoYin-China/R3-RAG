...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Pure VLM with CoT on ScienceQA")
    parser.add_argument('--data_root', type=str, required=True)
    parser.add_argument('--image_root', type=str, required=True)
    parser.add_argument('--output_root', type=str, default=None)
    parser.add_argument('--test_split', type=str, default='test')
    # 默认使用您验证过的 32B VL 模型名
    ...
    parser.add_argument('--label', type=str, default='pure_vlm_cot')
...
class PureVLMAgent:
    def __init__(self, args):
        ...
        try:
            ...
            if res.status_code != 200:
                ...
                sys.exit(1)
            print("✅ Model verified! Starting End-to-End inference.")
        except Exception as e:
    ...
    def solve(self, question, options, image_path, context, lecture, hint):
        ...
        # 3. 构造 CoT Prompt
        # 这里的关键是 Instruction，要求模型结合图片(如果有)进行思维链推理
        prompt = (
            f"{context_block}\n"
            f"Question: {question}\n"
            f"Options:\n{opt_lines}\n\n"
            "Instruction: You are an expert scientist. Solve this question step-by-step.\n"
            "1. If an image is provided, analyze its visual details carefully.\n"
            "2. Combine visual information with your scientific knowledge.\n"
            "3. Think through the logic step-by-step.\n"
            "4. Conclude with the final answer option.\n\n"
            "Format your response as:\n"
            "Reasoning: [Your detailed step-by-step logic]\n"
...
def main():
    args = parse_args()
    print(f"==== Running Pure Qwen2.5-VL CoT ====")
    ...
    agent = PureVLMAgent(args)
    correct = 0
    results = {}
    options = ["A", "B", "C", "D", "E"]
    
    print(f"🚀 Starting VLM CoT inference on {len(qids)} questions...")
